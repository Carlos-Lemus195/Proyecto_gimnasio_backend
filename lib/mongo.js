...
class MongoLib {
    ...
    realizarPago(collection, email, fechaPago) {
        return this.connect().then(db => {
            return db.collection(collection).updateOne({email: email}, { $set: {"moroso": "no", "montoMora": 0, "estado": "activo", "fechaPago": fechaPago}  }, { upsert: true });
